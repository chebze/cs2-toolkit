...
public sealed class EntityResolver
{
    ...
    public EntityResolver(ProcessMemory memory, GameOffsets offsets, ClairvoyanceOptions clairvoyanceOptions)
    {
        _memory = memory;
        _offsets = offsets;
    ...
    public MemoryState ReadState()
    {
        ...
        if (entityList == nint.Zero)
            return UnmatchedState(localPawn, localController, round: ReadRoundInfo(clientBase));

        var localTeam = ResolveTeam(localPawn, localController, entityList);
        var round = ReadRoundInfo(clientBase);
        ...
        var bomb = ResolveBombInfo(clientBase, entityList, players, bombSites);
    ...
    private BombInfo ResolveBombInfo(
        nint clientBase,
        nint entityList,
        ...
        BombSitesInfo bombSites)
    {
        ...
        if (gameRules == nint.Zero)
            return BombInfo.Hidden;

        if (_memory.Read<byte>(gameRules + _offsets.M_bBombPlanted) != 0)
            return ResolvePlantedBombInfo(clientBase, entityList, players, bombSites);
        ...
        if (_memory.Read<byte>(gameRules + _offsets.M_bBombDropped) != 0)
            return new BombInfo { Status = BombStatus.OnGround };
        ...
        foreach (var player in players)
        {
            ...
            if (HasActiveWeapon(pawn, entityList, GameOffsets.WeaponC4))
                return new BombInfo { Status = BombStatus.Equipped };
        ...
        return hasCarried
            ? new BombInfo { Status = BombStatus.Carried }
            : BombInfo.Hidden;
    }

    private BombInfo ResolvePlantedBombInfo(
        nint clientBase,
        nint entityList,
        List<PlayerInfo> players,
        BombSitesInfo bombSites)
    ...
    private BombInfo? TryResolvePlanting(nint entityList, List<PlayerInfo> players, BombSitesInfo bombSites)
    {
        foreach (var player in players)
        {
            if (player.Team != GameOffsets.TeamTerrorist || !player.IsAlive)
    ...
    private string? ResolvePlantingSiteLabel(nint pawn, nint entityList, BombSitesInfo bombSites)
    {
    ...
    private string? ResolveSiteLabel(nint entity, BombSitesInfo bombSites) =>
    ...
    private bool IsAnyPlayerDefusing(nint entityList, List<PlayerInfo> players)
    {
        foreach (var player in players)
        {
    ...
    private bool? ResolveDefuserHasKit(nint entityList, List<PlayerInfo> players, uint defuserHandle)
    {
    ...
    private nint ResolveDefuserPawn(nint entityList, List<PlayerInfo> players, uint defuserHandle)
    {
        if (defuserHandle is not (0 or 0xFFFFFFFF))
        {
            var defuserPawn = ResolveEntityFromHandle(entityList, defuserHandle);
    ...
    private List<PlayerInfo> CollectPlayers(nint entityList, nint localController)
    {
    ...
    private RoundInfo ReadRoundInfo(nint clientBase)
    {
        var gameRules = _memory.ReadPtr(clientBase + _offsets.DwGameRules);
        if (gameRules == nint.Zero)
            return RoundInfo.Empty;
    ...
    private static MemoryState UnmatchedState(
        nint localPawn,
        nint localController,
        int localTeam = 0,
