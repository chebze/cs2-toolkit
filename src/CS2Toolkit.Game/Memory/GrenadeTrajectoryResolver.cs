...
internal sealed class GrenadeTrajectoryResolver
{
    private const float MinThrowSpeedSquared = 10_000f;
    private readonly GrenadeTrajectorySimulator _simulator;
    private readonly GrenadeSimulationOptions _options;

    public GrenadeTrajectoryResolver(GrenadeSimulationOptions options)
    {
        _options = options;
    ...
    public bool IsGrenadeAimLikely(
        ...
        bool isInMatch)
    {
        if (!memory.IsAttached || !isInMatch || offsets.M_pWeaponServices == nint.Zero)
            return false;
        ...
        out string status)
    {
        ...
        const InternalGrenadeSource source = InternalGrenadeSource.ManualSimulation;
        ...
        out string status)
    {
        ...
        snapshot = BuildSnapshot(InternalGrenadeSource.GameTrail, weaponId, points, segments, bouncePoints, landing, bounces);
    ...
    {
        var position = BombSiteHelper.ReadEntityPosition(memory, offsets, localPawn);
        if (!position.IsValid)
        {
            eyePosition = default;
            return false;
